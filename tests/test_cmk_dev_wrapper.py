...
from cmk_dev_site.cmk_dev import get_all_tools
...
def test_wrapper_includes_all_entrypoints() -> None:
    ...
    cmk_dev_scripts = {
        ...
        if name.startswith("cmk-dev-") and name != "cmk-dev"
    }
...
def test_wrapper_tool_mappings_are_valid() -> None:
    """Test that all tools in the wrapper have valid main functions."""
    wrapper_tools = get_all_tools()

    for tool_name, tool_info in wrapper_tools.items():
        assert "main" in tool_info, f"Tool {tool_name} missing 'main' key"
        assert "description" in tool_info, f"Tool {tool_name} missing 'description' key"
        assert callable(tool_info["main"]), f"Tool {tool_name} 'main' is not callable"
        assert isinstance(tool_info["description"], str), (
            f"Tool {tool_name} description not a string"
        )
