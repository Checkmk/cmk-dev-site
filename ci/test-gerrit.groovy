            ...
            stage("Create changelog") {
                ...
                sh(label: "parse changelog", returnStdout: true, script: """
                    uv run \
                        ...
                        --additional_template_data '{"prerelease": "-rc${env.GERRIT_CHANGE_NUMBER}.${env.GERRIT_PATCHSET_NUMBER}"}' \
                        --print \
                        --output version.json
                """);
            }
                ...
                withCredentials([sshUserPrivateKey(credentialsId: "release-checkmk", keyFileVariable: 'keyfile')]) {
                    withEnv(["GIT_SSH_COMMAND=ssh -o \"StrictHostKeyChecking no\" -i ${keyfile} -l release"]) {
                        sh(label: "create and publish tag", returnStdout: true, script: """
                            ...
                            git tag -a v\$CHANGELOG_VERSION-rc${env.GERRIT_CHANGE_NUMBER}.${env.GERRIT_PATCHSET_NUMBER} -m "v\$CHANGELOG_VERSION-rc${env.GERRIT_CHANGE_NUMBER}.${env.GERRIT_PATCHSET_NUMBER}"
                            git tag --list
                            git push origin tag v\$CHANGELOG_VERSION-rc${env.GERRIT_CHANGE_NUMBER}.${env.GERRIT_PATCHSET_NUMBER}
                        """);
                    }
                }
            }
