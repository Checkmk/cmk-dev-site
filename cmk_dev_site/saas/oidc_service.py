...
from cmk_dev_site.saas.config import AdminPanelUrlConfig, OIDCConfig, write_model
...
from cmk_dev_site.utils import create_root_owned_file, is_port_in_use
...
def run() -> None:
    ...
    config = OIDCConfig()
    create_root_owned_file(OIDC_CONFIG_PATH)
    write_model(config, OIDC_CONFIG_PATH)

    admin_panel = AdminPanelUrlConfig()
    create_root_owned_file(ADMIN_PANEL_CONFIG_PATH)
    write_model(admin_panel, ADMIN_PANEL_CONFIG_PATH)
